...
ori_sentences=open("data/tsar/tsar2022_en_test_gold.tsv",encoding="utf-8").readlines()
...
def cal_freq_scores(substitutes):
    scores=[]
    for word1 in substitutes:
        tmp_score=zipf_frequency(word1, 'en')
...
word2vec_model_path = 'model/word2vec/cc.en.300.bin.gz'
...
def lexicalSubstitute(model,tokenizer, sentence, complex_word,prefix,suffix,beam):
    ...
    suffix1=" ".join(nltk.word_tokenize(suffix)[:3])
    ...
    not_candi = {'the', 'with', 'of', 'a', 'an', 'for', 'in', "-", "``", "*", "\"","it","","them","did","had","on"}
    ...
    for i3 in range(len(candis)):
        ...
        if "," in candi or "." in candi or "'" in candi or '"' in candi:
            continue
        candi_list=nltk.word_tokenize(candi)
        max_several=2
        now_several=0
    ...
    freq_scores=cal_freq_scores(bertscore_substitutes)
    glove_scores_static=cal_static_score(complex_word,bertscore_substitutes)

    real_prev_scores=0.04*torch.tensor(real_prev_scores)+0.04*freq_scores+0.1*glove_scores_static
...
def evaluate_lexical(model,tokenizer,ori_sentences,completed_steps):
    ...
    for i in tqdm(range(len(ori_sentences))):
        ...
        if again_again_prefix!=prefix:
            print("prefix need again and again tokenize")

    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            if prefix_ids==None or cur_len!=len(prefix_ids)+1:
...
#            print(next_token_scores)
            ...
            #if prefix_ids==None or cur_len<=len(prefix_ids)+1+complex_len-1:
                vocab_size = next_token_scores.shape[-1]
                next_token_scores = next_token_scores.view(batch_size, num_beams * vocab_size)
            ...
            finished_beams+=(beam_next_tokens==2)
            #finished_beams+=(beam_next_tokens==0)
            
            if tokenizer!=None and cur_len==len(prefix_ids)+2 and len(prefix_ids)!=0:
            #if tokenizer!=None and cur_len==len(prefix_ids)+1 and len(prefix_ids)!=0:
                target_tokens=list(map(lambda x:tokenizer.convert_ids_to_tokens(x),beam_next_tokens.cpu().tolist()))
                target_masks=torch.tensor(list(map(lambda x:not x.startswith("▁"),target_tokens)))
